...
def get_feedback_message(is_correct: bool, correct_answer: str, translations: list) -> str:
    ...
    if is_correct:
        return f'Правильно!\nСлово 🇺🇸 {correct_answer} переводится как 🇷🇺 {translations[0].translation}'
    else:
...
def send_feedback_message(
    ...
    result_icon = '✅' if is_answer_correct else '❌'
...
def update_user_word_setting(user_word_setting: UserWordSetting) -> None:
    ...
    user_word_setting.is_hidden = should_hide_word(user_word_setting, CHATBOT_DATA['correct_answers'])
